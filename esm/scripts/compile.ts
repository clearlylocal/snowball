...
for await (const x of Deno.readDir('./algorithms')) {
	if (x.isFile && x.name.endsWith(sblExt)) {
		...
		const outFilePath = `./esm/languages/${languageName}.js`

		if (languageMeta == null) {
			throw `No entry in languageCodes for language ${languageName}`
...
${
				content.replace(
					`${ctorName} = function`,
					`/** @type {import('../core/types.ts').StemmerConstructor} */
...
const getStemmerByLocale = String
	.raw`/** @typedef {Promise<import('./types.ts').StemmerConstructor} StemmerConstructor */
/** @typedef {Promise<import('./types.ts').Stemmer} Stemmer */
 ...
 * If not, ${'`null`'} is reterned for any work-in-progress stemmers.
 * @default false
 */
...
await writeTextFile('./esm/core/getStemmerByLocale.js', getStemmerByLocale)
